...
export function DeleteDocOrderModal({ open, onOpenChange, docOrderId, onSuccess }: DeleteDocOrderModalProps) {
    const deleteMutation = trpc.officer.docOrders.delete.useMutation({
        onSuccess: () => {
            onSuccess?.();
            onOpenChange(false);
        },
    ...
    const handleDelete = () => {
        deleteMutation.mutate({ id: docOrderId });
    };
        ...
        <Modal visible={open} animationType="fade" transparent onRequestClose={() => !deleteMutation.isPending && onOpenChange(false)}>
            ...
            <View className="flex-1 justify-center items-center bg-black/50 px-4">
                <View className="bg-card w-full max-w-sm rounded-3xl p-6 border border-border/50">
                    ...
                    <Text className="text-xl font-bold mb-2 text-foreground">Delete DOC Order?</Text>
