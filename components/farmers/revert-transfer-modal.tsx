...
export function RevertTransferModal({
    ...
    onOpenChange,
    onSuccess,
}: RevertTransferModalProps) {
    const mutation = trpc.officer.stock.revertTransfer.useMutation({
        onSuccess: () => {
            onOpenChange(false);
            onSuccess?.();
        },
                ...
                >
                    <View className="p-6 pb-2 flex-row justify-between items-center">
                        <View className="flex-row items-center gap-3">
                            ...
                            <View>
                                <Text className="text-xl font-bold text-foreground">Revert Transfer</Text>
                                <Text className="text-xs text-muted-foreground mt-0.5">
                                    Undo both sides of transfer
                                </Text>
                            </View>
                        </View>
                    ...
                    <View className="p-6 pt-2">
                        <View className="bg-muted/30 p-4 rounded-2xl border border-border/50 mb-6">
                            <Text className="text-xs font-bold text-muted-foreground uppercase mb-1">Transfer Note</Text>
                            <Text className="text-sm text-foreground font-medium">
                                {note || "No notes available for this transfer."}
                            </Text>
                        </View>

                        <Text className="text-sm text-muted-foreground text-center mb-6 leading-5">
                            This will reverse the stock deduction for the sender and reversal for the receiver. This action cannot be undone.
                        </Text>
                        ...
                        <View className="flex-row gap-3">
                            ...
                            >
                                <Text className="text-destructive-foreground font-bold">
                                    {mutation.isPending ? "Reverting..." : "Revert Transfer"}
                                </Text>
                            </Button>
                        </View>
                    </View>
