...
export function buildExcelWorksheet({ title, summaryData, rawDataTable, groupedDataTable, rawHeaders, definitions = [], mergePrimaryColumn = false, extraMerges = [] }: ExcelExportOptions) {
    try {
        ...
        console.error("Worksheet Build Error: ", error);
        throw new Error("Failed to build Excel worksheet");
    }
}
 ...
 */
export async function generateExcel(options: ExcelExportOptions): Promise<string> {
    try {
        const worksheet = buildExcelWorksheet(options);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Report");
...
/**
 * Generates a Multi-Sheet Excel file and returns its URI
 */
export async function generateMultiSheetExcel(sheets: { sheetName: string, options: ExcelExportOptions }[], title: string): Promise<string> {
    try {
        const workbook = XLSX.utils.book_new();
        for (const sheet of sheets) {
            const worksheet = buildExcelWorksheet(sheet.options);
            const safeName = sheet.sheetName.substring(0, 31).replace(/[\[\]\*\?\:\/\\]/g, '');
        ...
        console.error("Excel Multi-Sheet Generation Error: ", error);
        throw new Error("Failed to generate multi-sheet Excel file");
    }
}
...
export async function exportSalesLedgerExcel(sales: any[], title: string, returnOptions = false): Promise<any> {
    ...
    sales.forEach((s: any) => {
        ...
        const isEnded = s.cycleContext?.isEnded;
